...
the separation of a plane described by the XOR function  \rr{[Riferimento]}. 
...
To recognize patterns among a sequence of data such as video or audio, \textit{Long short-term memory} (LSTM)\rr{[Riferimento]}  and \textit{Gated Recurrent Unit} (GRU)\rr{[Riferimento]} are used
...
Deep Learning \rr{[Riferimento]} is a subfield of ML focalized in the study of deep neural networks.
...
Moreover, we employed as spatial distortion index the $D_sreg$\rr{[Riferimento]}, an implementation of the $D_s$ that takes advantages from the \textit{coefficient of determination} calculation. 
...
After that, a great descent would be reached\rr{[Che significa?]} and the learning rate would gradually degrade in order to avoid oscillations.
...
After that, stable phase would be achieved by performance \rr{[Che significa?]}.
...
As the model could have worse performance for data not trained for like in the real applications\rr{[Che significa?]}.
...
\begin{figure}[t]
    ...
    \caption{Learning curves obtained by varying $\beta$ with a 0.5 step in the range $[0,2]$ and $\alpha$ set to 0.5.\rr{[Nelle figure usi q2n nel testo Q2n oppure Q2N, in realtà sarebbe Q2$^n$. Per favore almeno tutti uguali.]}}
...
\rr{[Aggiungere una figura composta da 4 immagini: 
la GT e i prodotti finali ottenuti con GT training, RR training e NOREF training. Commentala!. Non so se vuoi visualizzarla in python o in Matlab, ma l'equalizzazione deve essere la stessa per le 4 figure, altrimenti, se le disegni una alla volta si hanno tutte equalizzazioni diverse. Se non lo sai fare (in Matlab basta vedere come funziona la parte finale di FusionAlgorithms quando usa shoImagesAll) fai prima un'unica immagine mettendo le 4 immagini una a fianco all'altra e stampa quella e poi ritagli. Qualcosa tipo\\
viewimage([GT GTTraining; RRtraining NOREFtraining])}
