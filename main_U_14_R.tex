...
\begin{figure}[t]
    ...
    \caption{Image by Maurice Peemen \rr{[Ti avevo scritto di mettere il riferimento, anche se solo un url!]}}
...
\begin{figure}
    ...
    \caption{Ground Truth used for validation\rr{[Ti ho detto di fare una figura in cui ci sono le 4 immagini in un array 2x2 (con un po' di spazio fra le figure, non accostate come la 4.15)]}  }
...
In Figs.~\ref{fig:comparison_gt},~\ref{fig:comparison_noref},~\ref{fig:comparison_rr} the images obtained with GT, NOREF and RR methods are illustrated, respectively.
...
\begin{figure}
    ...
    \caption{MSE between the ground truth and the final products obtained by employing the NOREF training (on the left) and the RR training (on the right). \rr{Ti ho detto di dividere le immagini}}
    \label{fig:diffcolors}
\end{figure}

For a better visual inspection, the \textit{mean square errors} (MSEs) related to the fianle products related to the NOREF and the RR training algorithms are showed in Fig.~\ref{fig:diffcolors}.
